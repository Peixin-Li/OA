...
class UserController extends Controller
{
  ...
  public function actiontestmsg() {
    ...
    **/
     ...
     *@return array  $data
     */
    public function actionSubscribe($page_tag='apply', $search="")
    {
        if(!empty($this->user))
        {
            //判断显示申购类型的标记
            ...
            if(  strlen($search) >= 2) {
                ...
                $sql_count .= ' and (goods_apply_detail.name like "%'. $search .'%" or goods_apply_detail.reason like "%'. $search .'%" )';
            }
            ...
            if(  strlen($search) >= 2) {
                $search = mysql_escape_string($search);
                $sql .= ' and (goods_apply_detail.name like "%'. $search .'%" or goods_apply_detail.reason like "%'. $search .'%" )';
            }
    ...
    public function actionSubscribeDetail($id)
    {
        if(!empty($this->user))
        {
            ...
            $procedure = GoodsApply::procedure($apply);
			      $this->render('subscribeDetail', array('apply'=>$apply,'procedure'=>$procedure));
        }
    }
    ...
    */
    ...
    public function actionPrintReimburse($id='')
    {
        ...
        $this->render('printReimburse',array('data'=>$data, 'department_name'=>$department_name));
    }
