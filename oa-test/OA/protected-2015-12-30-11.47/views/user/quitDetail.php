...
<div class="center">
	...
	<table class="table table-bordered left">
		<tbody>
			<tr>
				<th class="w130 center bg-fa">离职人姓名</th>
			...
			<tr>
				<th class="w130 center bg-fa">入职日期</th>
                <td><?php echo $apply->user->entry_day;?></td>
			</tr>
            <?php if(!empty($apply->quit_reason)): ?>
			<tr>
				<th class="w130 center bg-fa">离职原因</th>
			...
			<!-- 审批的人要填的东西 -->
			<tr>
				<th class="w130 center bg-fa">部门负责人审批</th>
				<td>
					<div class="pd5">
					...
					<div class="pd5" id="quit-date-div">
						建议于&nbsp;<input class="form-control w130 inline pointer center" id="quit-date" value="<?php if(!empty($quit_date)) echo $quit_date;?>">&nbsp;离职
					</div>
            ...
            <tr>
            	<th class="w130 center bg-fa">工作交接</th>
            	<td><button class="btn btn-success w130" onclick="showWorkTransform();">发起工作交接</button></td>
            </tr>
        	<?php elseif(!empty($apply->status) && $apply->status == "success" && $apply->handover_status!='create'): ?>
        	<tr>
            	<th class="w130 center bg-fa">工作交接</th>
            	<td><button class="btn btn-success w130" onclick="location.href='/user/deliverWorkDetail/id/<?php echo $apply->id;?>';">查看工作交接表</button></td>
            </tr>
        	<?php endif; ?>
		</tbody>
	</table>
	<?php if(!empty($user) && empty($apply->quit_reason) && $apply->next == $user->user_id): ?>
	<button class="btn btn-success w100 btn-lg" onclick="sendReason();">提交</button>
	<?php endif; ?>
	<?php if(!empty($user) && $user->user_id == $apply->next && $apply->user_id != $user->user_id): ?>
	<button class="btn btn-success btn-lg w100" onclick="sendProcess();">提交</button>
	<?php endif; ?>
</div>
<!-- 发起工作交接模态框 -->
...
<script type="text/javascript">
	// 显示发起工作交接模态框
	function showWorkTransform(){
		var ySet = (window.innerHeight - $("#work-transform-div").height())/2;
	    var xSet = (window.innerWidth - $("#work-transform-div").width())/2;
	    $("#work-transform-div").css("top",ySet);
	    $("#work-transform-div").css("left",xSet);
		$('#work-transform-div').modal({show:true});
	}
			...
			$.ajax({
		        ...
		        success:function(result){
		          if(result.code == 0){
		          	...
		          	var href_str = "/user/deliverWorkDetail/id/"+id;
