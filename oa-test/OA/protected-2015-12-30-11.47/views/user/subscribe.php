...
<div>
	  	...
	  	<table class="table table-bordered m0 center" id="subscribe-any-table">
	  		...
	  		<tbody>
	  			<tr class="new-subscribe-tr">
	  				...
	  				<td>
	  					<!-- <button class="btn btn-default" onclick="newLine(this);">新增</button> -->
	...
	<!-- 费用申请记录 -->
	<div class="pd20 bor-1-ddd" id="new-sub">
		<div class="pb20 pl5 mb15">
		    ...
		    <div id="new-sub2">
		    	<!-- 费用申请、报销类型切换标签 -->
			  	...
			  	<!-- 费用申请表格 -->
			  	<div  id="subscribe-table" class="bor-1-ddd pd20">
			  		<!-- <div id="new-subscribe-p">
						<h4>
					...
					<div id="new-subscribe-search_add" class="h40 mb10">
                        ...
                        <div class="fl ml20">
                            <label>搜索：</label>
                            <input class="form-control inline w200" placeholder="请输入搜索条件" id="search-condition" value="<?php echo $search_condition ?>">
				  	...
				  	<table class="table m0 table-bordered center" id="subscribe-table-new">
						...
						<?php foreach ($goods_apply_list as $key): ?>
							<tr class="reimburse-detail-tr">
								...
								<td class="w40"><input type="checkbox" class="checkbox" style="margin: 0 auto"></td>
								<td class="w120">
								...
								<td class="w80"><?php echo $key['name'] ?></td>
				  	...
				  	<button id="batch" class="btn btn-default mt15" onclick="newShowReimburse3(document.getElementById('tbody'))">批量生成报销单</button>
			  		<!-- 分页 -->
			  		<div class="mt20">
				        <?php 
			  	...
			  	<!-- 费用报销表格 -->
			  	<div id="reimburse-div" class="hidden bor-1-ddd">
							...
							<?php foreach ($history_reimburses as $key): ?>
								<tr>
									...
									<td>
										...
										<?php elseif($key['status']=='wait'): ?>
										<button class="btn btn-success" onclick="showReimburseChange()">提交</button>
		  	...
		  	<!-- 报销单修改页面 -->
		  	<div class="bor-1-ddd" id="reimburse-change-div">
				<div class="w800 m0a mt50 pb50">
					<div class="center">
						<h3>报销清单</h3> 
					</div>
					<table class="table table-bordered center">
						...
						<tbody>
							<tr>
								<td id="reimburse-change-type">办公费</td>
								<td id="reimburse-change-name">快递费</td>
								<td id="reimburse-change-date">2015-12-30 09:53:06</td>
								<td id="reimburse-change-state">邮寄到公司</td>
								<td id="reimburse-change-price">500.00</td>
							</tr>
						</tbody>
					</table>
					<div>
						...
						<input type="text" class=" ml10 mb30 form-control inline w100">
						<p>支付方式</p>
						...
						<div id="reimburse-change-transfers" class="mt25">
							<table>
								<tbody>
									<tr class="">
										<td><p class="inline">收款人</p></td>
										<td><input type="text" class="ml20 w100 mt5 mb5"></td>
									</tr>
									<tr class="">
										<td><p class="inline">开户银行</p></td>
										<td><input type="text" class="ml20 w130 mt5 mb5"></td>
									</tr>
									<tr class="">
										<td><p class="inline">账号</p></td>
										<td><input type="text" class="ml20 w300 mt5 mb5"></td>
									</tr>
								</tbody>
							</table>
						</div>
						<div id="reimburse-change-borrowing" class="mt25">
							<table>
								<tbody>
									<tr>
										<td><p class="inline">原借款金额</p></td>
										<td><input type="text" class="ml20 w130"></td>
									</tr>
								</tbody>
							</table>
						</div>
...
/*-------------------------------------------------------确认报销单----------------------------------------------------------------------*/
	...
	function cancelSendReimburse(){
		$("#reimburse-bill-div").fadeOut(400,function(){
...
<script type="text/javascript">
	// var categoryTest = <?php echo CJSON::encode($reimburses) ?>;
	// var categoryTest2 = <?php echo CJSON::encode($categorys) ?>;
	// console.log(categoryTest2);
	...
	function changeApply(row){
		...
		// // $('.change-type') = row.children[0].innerText;
	...
	// if($("#reimburse-type-select").val() == 'all'){
	// 	$('#batch').addClass('hidden');
	// }
	// var list = <?php echo CJSON::encode($history_reimburses) ?>;
	// console.log(list);
	...
	function newShowReimburse3(row){
		...
		$("tr.reimburse-detail-tr").each(function(){
			if(this.children[2].children[0].checked == true){
				...
				if($.inArray(typeNow,typeNow_Arr) == -1){
					...
					// console.log(arr.typeNow.reimburse_detail_arr);
					...
					// 		dataType:'json',
					// 		url:'/ajax/reimburse',
					// 		data:{'way':"", 'bank_info':"", 'bank_code':"", 'payee':"", 'borrow_amount':"", 'receipt_num':"", 'details':reimburse_detail_arr},
					// 		success:function(result){
					// 			if(result.code == 0){
					// 				showHint("提示信息","提交报销单成功！");
					// 				setTimeout(function(){location.href="/user/subscribe/page_tag/reimburse"},1200);
		...
		for(var i=0; i<typeNow_Arr.length;i++){
			//提交报销单接口
			console.log(arr[typeNow_Arr[i]].reimburse_detail_arr);
			$.ajax({
				type:'post',
				dataType:'json',
				url:'/ajax/reimburse',
    ...
    function searchGoodsApply() {
        location.href = '/user/subscribe?search='+ $("#search-condition").val();
    }

    function showReimburseChange(){
