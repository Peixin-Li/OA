...
echo "console.log('leaveSummaryFailed');";
echo "</script>";
?>
...
<div class="bor-1-ddd">
	...
	<ul class="nav nav-tabs" role="tablist">
        ...
        <li role="presentation" class="active"><a href="/oa/leaveSummaryFailed">未通过</a></li>
    </ul>
    <!-- 请假记录 -->
    ...
    <table class="table table-striped table-bordered table-hover">
     ...
     $types = array('casual'=>'事假','sick'=>'病假','funeral'=>'丧假','marriage'=>'婚假','maternity'=>'产假','annual'=>'年假','compensatory'=>'补假','others'=>'其他假');
        foreach($LeaveSummarys as $msg){
            ...
            {
                echo "<td><a href='/oa/msg/leave/{$msg->leave_id}/type/leaveSummaryFailed'>{$msg->user->cn_name}---".(empty($types[$msg->type])?'':$types[$msg->type])."---时间:&nbsp&nbsp<b>".date('Y-m-d H:i', strtotime($msg->start_time))."</b>&nbsp到&nbsp<b>".date('H:i', strtotime($msg->end_time))."</b></a></td>";
            }
            else
            {
                echo "<td><a href='/oa/msg/leave/{$msg->leave_id}/type/leaveSummaryFailed'>{$msg->user->cn_name}---".(empty($types[$msg->type])?'':$types[$msg->type])."---时间:&nbsp&nbsp<b>".date('Y-m-d', strtotime($msg->start_time))."</b>&nbsp到&nbsp<b>".date('Y-m-d', strtotime($msg->end_time))."</b></a></td>";
            }
        ...
        <div class="overflow-a xh400">
            ...
            <?php
                    for($i=1;$i<=$total;$i++){
                        ...
                        {
                            echo "<a class='btn btn-default btn-block left mt10 active' href='/oa/leaveSummaryFailed?page=".$i."'>".$i."</a>";
                        }
                        else
                        {
                            echo "<a class='btn btn-default btn-block left mt10' href='/oa/leaveSummaryFailed?page=".$i."'>".$i."</a>";
                        }
                    }
            ?>
        </div>
