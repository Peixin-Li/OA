...
echo "console.log('leaveSummaryWait');";
echo "</script>";
?>
...
<div class="bor-1-ddd">
    ...
    <div class="m0 p0 pd20">
        ...
        <div class="mb15">
            ...
            <input class="form-control w130 inline" style="cursor:pointer;" onclick="setmonth(this,'yyyy-MM','2014-10-1','2014-10-2',1)" id="search_date" ></input>
            ...
            <button class="btn btn-success mt-5 pd5 w80 ml10" onClick="leaveSummarySearch();">查询</button>
        </div>
    <!-- 可选标签 -->
	<ul class="nav nav-tabs" role="tablist">
        <li role="presentation" class="active"><a href="/oa/leaveSummaryWait">待审批</a></li>
    ...
    <table class="table table-striped table-bordered table-hover">
     ...
     $types = array('casual'=>'事假','sick'=>'病假','funeral'=>'丧假','marriage'=>'婚假','maternity'=>'产假','annual'=>'年假','compensatory'=>'补假','others'=>'其他假');
        foreach($LeaveSummarys as $msg){
            ...
            {
                echo "<td><a href='/oa/msg/leave/{$msg->leave_id}/type/leaveSummaryWait'>{$msg->user->cn_name}---".(empty($types[$msg->type])?'':$types[$msg->type])."---时间:&nbsp&nbsp<b>".date('Y-m-d H:i', strtotime($msg->start_time))."</b>&nbsp到&nbsp<b>".date('H:i', strtotime($msg->end_time))."</b></a></td>";
            }
            else
            {
                echo "<td><a href='/oa/msg/leave/{$msg->leave_id}/type/leaveSummaryWait'>{$msg->user->cn_name}---".(empty($types[$msg->type])?'':$types[$msg->type])."---时间:&nbsp&nbsp<b>".date('Y-m-d', strtotime($msg->start_time))."</b>&nbsp到&nbsp<b>".date('Y-m-d', strtotime($msg->end_time))."</b></a></td>";
            }
        ...
        <div class="overflow-a xh400">
            ...
            <?php
                    for($i=1;$i<=$total;$i++){
                        ...
                        {
                            echo "<a class='btn btn-default btn-block left mt10 active' href='/oa/leaveSummaryWait?page=".$i."'>".$i."</a>";
                        }
                        else
                        {
                            echo "<a class='btn btn-default btn-block left mt10' href='/oa/leaveSummaryWait?page=".$i."'>".$i."</a>";
                        }
                    }
            ?>
        </div>
