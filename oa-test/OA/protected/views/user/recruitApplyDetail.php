...
<div>
    <!-- 标题 -->
    <h4 class="bor-l-1-ddd bor-r-1-ddd m0 pd20">
        <strong>招聘申请详情</strong>
    </h4>
    <!-- 进度条 -->
	<div class="bor-l-1-ddd bor-r-1-ddd hidden">
	...
	<table class="table table-bordered m0">
        ...
        <tr>
            <th class="w130 center">简历</th>
            <td>
                <?php if(!empty($resumes)): ?>
                <table class="table table-unbordered center m0 " id="resume-table">
                    <thead>
                        <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <a target="_blank" href="<?php echo '/user/viewResume/id/'.$resume->id; ?>"><?php echo "{$resume->apply->title}-{$resume->name}-{$resume->source}-".date('Ymd',strtotime($resume->create_time)).substr($resume->resume_file,8); ?></a>
                            <a href="<?php echo "/user/downloadResume/id/{$resume->id}";?>">下载</a>
                        </td>
            <?php if($resume->status == 'create'): // 刚刚创建  ?>
                        <td>
            ...
            <?php elseif($resume->status == 'conform'): // 已经确认可以安排面试 ?>
                        <td>
                            <?php if(!$tag): // 当前登陆的是HR ?>
            ...
            <?php elseif($resume->status == 'inconformity'): // 确认不面试 ?>
                        <td>不符合要求</td>
            <?php elseif($resume->status == 'arrange' && empty($resume->interviewer)): // 已确定面试时间，但未安排面试官 ?>         
                        <td>
            ...
            <?php elseif($resume->status == 'arrange' && !empty($resume->interviewer)): // 已确定面试时间并且已安排面试官 ?>
                        <td>
                            面试时间：<?php echo date('Y年m月d日 H:i', strtotime($resume->interview_time)); ?>
                            <?php if($tag): ?>
                            ...
                            面试官：<?php $interviewer_apply = $resume_interviewer[$key]; if(!empty($interviewer_apply)) echo $interviewer_apply->cn_name; ?>
                            </br>
                            <?php if($tag):?>
            ...
            <?php elseif($resume->status == 'nonarrival'): // 缺席 ?>
                        <td>没有来面试</td>
            <?php elseif($resume->status == 'giveup'): // 已通过面试但放弃入职 ?>
                        <td>放弃入职</td>
            <?php elseif($resume->status == 'success'): // 已面试，但未创建评估表 ?>
                        <td>
                            <?php if(!$tag): ?>
                            已面试，待填写评估表
                            <?php else: ?>
                            <a href="/user/interviewEvaluateDetail/id/<?php echo $resume->id;?>">已面试，填写评估表</a>
                            <?php endif; ?>
                        </td>
            <?php elseif($resume->status == 'assessment'): // 已面试并且已创建评估表 ?>
                        <td>
                            <?php if(!$tag): ?>
                            <a href="/user/interviewEvaluateDetail/id/<?php echo $resume->id;?>">请填写评估表</a>
                            <?php else: ?>
                            <a href="/user/interviewEvaluateDetail/id/<?php echo $resume->id;?>">查看评估表</a>
                            <?php endif; ?>
                        </td>
            <?php elseif($resume->status == 'entry'): // 已面试并且已通过 ?>
                        <td>
                            <?php if(!$tag): ?>
                            <a href="/oa/interviewEvaluateDetail/id/<?php echo $resume->assessment->id;?>" title="查看评估表">即将入职</a>
                            <?php else: ?>
                            <a href="/oa/interviewEvaluateDetail/id/<?php echo $resume->assessment->id;?>" title="查看评估表">即将入职</a>
            ...
            success:function(data){
                if(data.code == 0){
                    ...
                    if(status == "conform"){
                        ...
                        <?php if($tag):?>
                        var str = "<a href='/user/interviewEvaluateDetail/id/"+id+"'>已面试，填写评估表</a>";
                        $(obj).parent().html(str);
                        <?php else: ?>
                        var str = "已面试，待填写评估表";
                        $(obj).parent().html(str);
...
/*----------------------------------------------------上传简历----------------------------------------------------------*/
	...
	function showSelectedFile(){
        ...
        var file_type_tag = 0; // 文件类型标记
        var num = 1; // 文件数

        // 遍历文件数组
		$.each(file_arr, function(key,value){
            if(value != null){
                var file_type = this.type; // 文件类型
                if(file_type.indexOf("officedocument") > -1 || file_type.indexOf("word") > -1){  // 判断word文档
        ...
        xhr.onreadystatechange=function(){
            if(xhr.readyState==4 && xhr.status==200){
                ...
                if(code == 0){
                    ...
                    $("#tr-"+(index+1)).children().last().html("<span class='glyphicon glyphicon-remove b2'></span>&nbsp;上传失败");
                    fault_tag = 1;
                }else if(code == -2){
                    $("#tr-"+(index+1)).children().last().html("<span class='glyphicon glyphicon-remove b2'></span>&nbsp;参数错误");
                    fault_tag = 1;
                }else if(code == -3){
                    $("#tr-"+(index+1)).children().last().html("<span class='glyphicon glyphicon-remove b2'></span>&nbsp;找不到该申请");
                    fault_tag = 1;
                }else if(code == -4){
                    $("#tr-"+(index+1)).children().last().html("<span class='glyphicon glyphicon-remove b2'></span>&nbsp;文件类型错误");
                    fault_tag = 1;
                }else if(code == -5){
                    $("#tr-"+(index+1)).children().last().html("<span class='glyphicon glyphicon-remove b2'></span>&nbsp;大小超过5M");
                    fault_tag = 1;
                }else{
