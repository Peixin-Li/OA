...
echo "console.log('businessTrip_summary_wait');";
echo "</script>";
?>
...
<div class="bor-1-ddd">
    ...
    <div class="m0 p0 pd20">
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" class="active"><a href="/oa/businessTrip_summary_wait">待审批</a></li>
        ...
        <?php
            foreach($msgs as $msg){
                ...
                if(date('Y-m-d', strtotime($msg->start_time)) == date('Y-m-d', strtotime($msg->end_time))){
                     echo "<td><a href='/oa/outMsg/out/{$msg->out_id}/type/businessTrip_summary_wait'>{$msg->user->cn_name}--<b>出差地点:</b>&nbsp{$msg->place}---<b>出差时间:</b>&nbsp<b>".date('Y-m-d H:i', strtotime($msg->start_time))."到".date('H:i', strtotime($msg->end_time))."</a>";
                }
                else{
                    echo "<td><a href='/oa/outMsg/out/{$msg->out_id}/type/businessTrip_summary_wait'>{$msg->user->cn_name}--<b>出差地点:</b>&nbsp{$msg->place}---<b>出差时间:</b>&nbsp<b>".date('Y-m-d', strtotime($msg->start_time))."到".date('Y-m-d', strtotime($msg->end_time))."</a>";
                }
                echo "<br>";
            ...
            <?php
                for($i=1;$i<=$total;$i++){
                    if($page->currentPage+1 == $i){
                        echo "<a class='btn btn-default btn-block left mt10 active' href='/oa/businessTrip_summary_wait?page=".$i."'>".$i."</a>";
                    }else{
                        echo "<a class='btn btn-default btn-block left mt10' href='/oa/businessTrip_summary_wait?page=".$i."'>".$i."</a>";
                    }
                }
            ?>
        </div>
