...
class GoodsApply extends CActiveRecord
{
    ...
    public static function addApplyAndDetail($data, $arr)
    {
        ...
        foreach($arr as $row)
        {
            ...
            if( isset($row['fee_div_p']) )
                $row['fee_div_p'] = CJSON::encode($row['fee_div_p']);
    ...
    public static function addGoodsApply($arr , $user, $tag)
    {
        ...
        try
        {
            ...
            //去除审批流程中的自己审批的节点
            $first_fee_div = $arr[0]['fee_div_p'];
            if( count($first_fee_div) == 1) {
                foreach($first_fee_div as $key=>$value) {
                    if( $project_info = Project::model()->findByPk($key) ) {
                        if( $user_id = Users::model()->findByPk($project_info->project_admin)->user_id ){
                            if(!in_array($user_id, $procedure_list)) {
                                array_unshift($procedure_list, $user_id);
                            }
                        }
                    }
                }
