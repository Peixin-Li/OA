...
//        $_POST = array('id'=>12, 'contents'=>array('工作内容','PHP代码'));
    ...
    */
    ...
    'name' => '修改'
     */
    public function actionEditGoodsApply()
    {
        $id = empty($_POST['id']) ? '' : $_POST['id'];
        $data['category'] = empty($_POST['category']) ? '' : htmlspecialchars($_POST['category']);
        $data['type'] = empty($_POST['type']) ? '' : htmlspecialchars($_POST['type']);
        $data['name'] = empty($_POST['name']) ? '' : htmlspecialchars($_POST['name']);
        // $data['fee_div_name'] = empty($_POST['fee_div_name']) ? '' : htmlspecialchars($_POST['fee_div_name']);
        // $fee_div_p = empty($_POST['fee_div_p']) ? array() : $_POST['fee_div_p'];
        $data['price'] = empty($_POST['price']) ? '' : (float)$_POST['price'];
        $data['reason'] = empty($_POST['reason']) ? '' : htmlspecialchars($_POST['reason']);

        $response = array('code'=>-1 , 'msg'=>'edit apply failed');
        if( !preg_match('/^[1-9]\d*$/', $id) || empty($data['category']) || empty($data['name']) || empty($data['price']) || empty($data['reason']))
        {
            $response['code'] = -2;
            $response['msg'] = 'param error'.$id;
        }
        // elseif ( empty($data['fee_div_name']) ) {
        //     $response['code'] = -2;
        //     $response['msg'] = 'param error2';
        // }
        // elseif(!$data['fee_div_p'] = GoodsApplyDetail::checkFeeDiv($fee_div_p)) {
        //     $response['code'] = -2;
        //     $response['msg'] = 'param error3';
        // }
        elseif(!$apply_detail = GoodsApplyDetail::model()->findByPk($id))
        {
            $response['code'] = -3 ;
        ...
        else if( $data['price'] > ($apply_detail->price*(int)$apply_detail->quantity) ) {
            $response['code'] = -6 ;
            $response['msg']  = 'price error';
        }
        else if( GoodsApplyDetail::model()->updateByPk($id, $data) ) {    //修改申请单
            $response['code'] = 0;
            $response['msg']  = 'ok';
        }
        echo CJSON::encode($response);
    ...
    /*
        新建项目 yeqingwen 2015-12-30
        serial_number   ：string     项目编号    not null
        name            ：string     项目名称    not null
        department_id   ：int        所属部门ID  not null
        serial_number   ：int        项目负责人  not null
        remark          : string     备注说明
    */
    public function actionAddProject() {
        $data['serial_number'] = empty($_POST['serial_number']) ? '' : htmlspecialchars($_POST['serial_number']);
        $data['name'] = empty($_POST['name']) ? '' : htmlspecialchars($_POST['name']);
        $data['department_id'] = empty($_POST['department_id']) ? 0 : (int)($_POST['department_id']);
        $data['project_admin'] = empty($_POST['project_admin']) ? 0 : (int)($_POST['project_admin']);
        $data['remark'] = empty($_POST['remark']) ? "" : htmlspecialchars($_POST['remark']);
        $data['create_time'] = date('Y-m-d H:i:s');
        
        $response = array('code'=>-1, 'msg'=>'failed');
        if( empty($data['name']) || empty($data['department_id']) || empty($data['project_admin']) ) {
            $response['code'] = -2;
            $response['msg'] = 'parmas error';
        }
        elseif ( Yii::app()->session['user_id'] != Users::getCeo()->user_id) {
            $response['code'] = -98;
            $response['msg'] = 'permission denied';
        }
        elseif ( !$deparment_info = Department::model()->findByPk($data['department_id']) ) {
            $response['code'] = -3;
            $response['msg'] = 'department not exsist';
        }
        elseif ($deparment_info->department_status !="display") {
            $response['code'] = -3;
            $response['msg'] = 'department been delete';
        }
        elseif ( !$admin_info = Users::model()->findByPk($data['project_admin']) ) {
            $response['code'] = -4;
            $response['msg'] = 'user not exsist';
        }
        elseif ($admin_info->status !="work") {
            $response['code'] = -4;
            $response['msg'] = 'user been delete';
        }
        else if( Project::addProject($data) ){
            $response['code'] = 0;
            $response['msg'] = 'ok';
        }
        echo CJSON::encode($response);
    ...
    /*
        删除项目 yeqingwen 2015-12-30
        project_id      : int        项目ID      not null
        只有当project的 public_ps 为零才可以删除
    */
    public function actionDelProject() {
        $project_id = empty($_POST['project_id']) ? 0 : $_POST['project_id'];
        $response = array('code'=>-1, 'msg'=>'failed');
        if( empty($project_id) ) {
            $response['code'] = -2;
            $response['msg'] = 'parmas error';
        ...
        elseif (!$project_info = Project::model()->findByPK($project_id) ) {
            $response['code'] = -4;
            $response['msg'] = 'can not find project';
        }
        elseif ( !FeeDivisionTpl::delProjectInfo($project_id) ) {
            $response['code'] = -6;
            $response['msg'] = 'public_ps not zero';
        }
        else {
            $project_info->enable = 'no';
    ...
    */
    public function actionEditProject() {
        $project_id = empty($_POST['project_id']) ? 0 : (int)($_POST['project_id']);
        $data['serial_number'] = empty($_POST['serial_number']) ? '' : htmlspecialchars($_POST['serial_number']);
        $data['name'] = empty($_POST['name']) ? '' : htmlspecialchars($_POST['name']);
        $data['department_id'] = empty($_POST['department_id']) ? 0 : (int)($_POST['department_id']);
        $data['project_admin'] = empty($_POST['project_admin']) ? 0 : (int)($_POST['project_admin']);
        ...
        else if( Project::model()->updateByPk($project_id, $data) ){
            $response['code'] = 0;
            $response['msg'] = 'ok';
        }
        echo CJSON::encode($response);
    ...
    /*
        添加费用分摊模板 yeqingwen 2015-12-31
        name            ：string     模板名称      not null
        fee_div_p       ：array      项目分摊比例  not null
        remark          : string     备注说明
    */
    public function actionAddFeeTpl(){
        // $_POST = array('name'=>'测试tpl', 'remark'=>'231', 'fee_div_p'=>array(4=>50,5=>50));
        $data['name'] = empty($_POST['name']) ? '' : htmlspecialchars($_POST['name']);
        $data['fee_div_p'] = empty($_POST['fee_div_p']) ? array() : $_POST['fee_div_p'];
        $data['remark'] = empty($_POST['remark']) ? "" : htmlspecialchars($_POST['remark']);
        
        $response = array('code'=>-1, 'msg'=>'failed');
        if( empty($data['name']) || empty($data['fee_div_p']) ) {
            $response['code'] = -2;
            $response['msg'] = 'parmas error';
        }
        elseif ( Yii::app()->session['user_id'] != Users::getCeo()->user_id) {
        ...
        elseif ( !FeeDivisionTpl::verification($data['fee_div_p']) ) {
            $response['code'] = -3;
            $response['msg'] = 'fee_div_p error';
        }
        else if( FeeDivisionTpl::addTpl($data) ){
            $response['code'] = 0;
            $response['msg'] = 'ok';
        }
        echo CJSON::encode($response);
    ...
    /*
        修改费用分摊模板 yeqingwen 2015-12-31
        tpl_id          ：int        模板ID      not null
        name            ：string     模板名称      not null
        fee_div_p       ：array      项目分摊比例  not null
        remark          : string     备注说明
    */
    public function actionEditFeeTpl() {
        // $_POST = array('tpl_id'=>2,'name'=>'测试tpl', 'remark'=>'231s ', 'fee_div_p'=>array(4=>40,5=>60));
        $tpl_id = empty($_POST['tpl_id']) ? 0 : (int)$_POST['tpl_id'];
        $data['name'] = empty($_POST['name']) ? '' : htmlspecialchars($_POST['name']);
        $data['fee_div_p'] = empty($_POST['fee_div_p']) ? array() : $_POST['fee_div_p'];
        $data['remark'] = empty($_POST['remark']) ? "" : htmlspecialchars($_POST['remark']);
        
        $response = array('code'=>-1, 'msg'=>'failed');
        if( empty($data['name']) || empty($data['fee_div_p']) || empty($tpl_id) ) {
            $response['code'] = -2;
            $response['msg'] = 'parmas error';
        }
        elseif ( Yii::app()->session['user_id'] != Users::getCeo()->user_id) {
        ...
        elseif ( !$data['fee_div_p'] = CJSON::encode($data['fee_div_p']) ) {
            $response['code'] = -3;
            $response['msg'] = 'fee_div_p error';
        }
        else if( FeeDivisionTpl::model()->updateByPk($tpl_id,$data) ){
            $response['code'] = 0;
            $response['msg'] = 'ok';
        }
        echo CJSON::encode($response);
    ...
    /*
        删除费用分摊模板 yeqingwen 2015-12-31
        tpl_id          ：int        模板ID      not null
    */
    public function actionDeleteFeeTpl() {
        // $_POST = array('tpl_id'=>2);
        $tpl_id = empty($_POST['tpl_id']) ? 0 : (int)$_POST['tpl_id'];
        ...
        else if( FeeDivisionTpl::model()->deleteByPk($tpl_id) ){
            $response['code'] = 0;
            $response['msg'] = 'ok';
        }
        echo CJSON::encode($response);
    ...
    /*
        删除费用分摊模板 yeqingwen 2015-12-31
        user             string        用户名      not null
        old_pwd          string        旧密码      not null
        new_pwd          string        新密码      not null
    */
    public function actionChangePassword() {
        // $_POST = array('user'=>'test2', 'old_pwd'=>'123456', 'new_pwd'=>'123456@#\"');
        $user = empty($_POST['user'])? '' : $_POST['user'];
        ...
        $ad_file = getcwd() .'/ad_pwd.sh';
        $command = ' /usr/bin/expect '.$ad_file.' "' .$user. '" "'.$old_pwd.'" "'.$new_pwd.'" ';
        $response = array('code'=>-1, 'msg'=>'failed');

        if( empty($user) || empty($old_pwd) || empty($new_pwd) ) {
            $response['code'] = -2;
            $response['msg'] = 'params error';
        }
        else if ( !exec($command, $result) || empty($result) ) {
            $response['code'] = -3;
            $response['msg'] = 'command exec failed';
        }
        //用户名或密码错误
        elseif( strpos( end($result), 'NT_STATUS_LOGON_FAILURE' ) ) {
            $response['code'] = -5;
            $response['msg'] = 'user or password error';
        }
        elseif( strpos( end($result), 'changed for' ) ) {
            $response['code'] = 0;
            $response['msg'] = 'ok';
        }
        else
            $response['msg'] = end($result);
        echo CJSON::encode($response);
    }
