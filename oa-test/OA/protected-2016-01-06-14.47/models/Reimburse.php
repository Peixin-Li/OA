        ...
        try
        {
          //添加银行卡信息
            ...
            foreach($details as $row)
            {
                ...
                //根据申请详情来计算项目分摊的费用
                if ( $fee_div_p = CJSON::decode($detail['fee_div_p'], true) ) {
                    foreach ($fee_div_p as $project_id => $persent) {
                        if( isset($fee_div[$project_id]) )
                            $fee_div[$project_id] += ( (int)$persent ) * $row['amount'] / 100 ;
                        else
                            $fee_div[$project_id] = ( (int)$persent ) * $row['amount'] / 100 ;
                    }
                }
            ...
            $data['fee_div'] = CJSON::encode($fee_div);
