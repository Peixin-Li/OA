...
<div class="bor-1-ddd">
    <!-- 标题 -->
    <h4 class="pd10 m0 b33 bor-b-1-ddd">消息列表</h4>

    <div class="m0 p0 pd20">
    <!-- 可选标签 -->
    <ul class="nav nav-tabs" role="tablist">
        <li role="presentation" <?php if($status==''){ echo "class='active'"; } ?>><a href="/oa/msgs">全部</a></li>
        <li role="presentation" <?php if($count_wait>0){if($status=='wait'){ echo "class='active'";}}else{ echo "class='hidden'";} ?>><a href="/oa/msgs/status/wait">未读(<?php echo $count_wait;?>)</a></li>
    </ul>
    <!-- 消息列表     -->
    <?php if(!empty($msgs)): ?>
    ...
    <div id="page" class="w100%">
        ...
        <div class="inline-block w300">
            ...
            <?php else: ?>
            <button class="btn btn-success pd5 hidden disabled" onclick="markAllRead();">全部标为已读</button>
            <?php endif; ?>
        </div>
        <!-- 分页 -->
            ...
            <?php
                if($status=='wait'){
                    for($i=1;$i<=$total;$i++){
                        ...
                        {
                            echo "<a class='btn btn-default btn-block left mt10 active' href='/oa/msgs?page=".$i."'>".$i."</a>";
                        }
                        else
                        {
                            echo "<a class='btn btn-default btn-block left mt10' href='/oa/msgs?page=".$i."'>".$i."</a>";
                        }
                    }
                } 
                else{
                    for($i=1;$i<=$total;$i++){
                        if($page->currentPage+1 == $i)
                        {
                            echo "<a class='btn btn-default btn-block left mt10 active' href='/oa/msgs?status=read&page=".$i."'>".$i."</a>";
                        }
                        else
                        {
                            echo "<a class='btn btn-default btn-block left mt10' href='/oa/msgs?status=read&page=".$i."'>".$i."</a>";
                        }
                    }
                }
            ?>
...
<script type="text/javascript">
    ...
    function goToMsg(obj, id){
        var href_str = 'oa/msgDetail/id/'+id;//原来是user/msgDetail/...
