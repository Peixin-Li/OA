...
<div id="add-Tpl-div" class="modal fade in hint bor-rad-5 w600">
    ...
    <div class="modal-footer center" id="modal-footer">
            ...
            <button class="btn btn-default w100" data-dismiss="modal" onclick="">取消</button>
    </div>
</div>
...
<script type="text/javascript">
	...
	function changeApply(row){
		// console.log(row.children[10].innerText.trim());
	...
	function changeApplyCommit(){
		...
		if(type1 == storeArray['type1'] && second_type == storeArray['type2'] && name == storeArray['name'] && reason == storeArray['state']){
			if(price < storeArray['price']){
				$.ajax({
	    			type:'post',
	    			dataType:'json',
	    			url:'/ajax/EditGoodsApply',
	    			data:{'id':id, 'category':type1,'type':second_type,'price':price,'reason':reason,'name':name},
	    			success:function(result){
